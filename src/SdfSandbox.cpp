...
namespace Demo
{
	class SdfSandboxGraphicsSystem : public GraphicsSystem
	{
		virtual Ogre::CompositorWorkspace *setupCompositor()
		{
			Ogre::CompositorManager2 *compositorManager = mRoot->getCompositorManager2();
			return compositorManager->addWorkspace( mSceneManager, mRenderWindow->getTexture(), mCamera,
													"SdfRenderer/Workspace", true );
		}
	...
	public:
		SdfSandboxGraphicsSystem( GameState *gameState ) : GraphicsSystem( gameState )
		{
			...
			//	%APPDATA%/SdfSandbox/ on Windows
			//	~/.config/SdfSandbox/ on Linux
			//	macCachePath() + "/SdfSandbox/" (NSCachesDirectory) on Apple -> Important because
			...
			//	reset.
			//  Obviously you can replace "SdfSandbox" by your app's name.
#if OGRE_PLATFORM == OGRE_PLATFORM_WIN32
			mWriteAccessFolder = +"/";
			TCHAR path[MAX_PATH];
...
#	endif
			...
			mWriteAccessFolder = macCachePath() + "/SdfSandbox/";
			// Create "pathToCache/SdfSandbox"
			mWriteAccessFolder += "/SdfSandbox/";
			result = mkdir( mWriteAccessFolder.c_str(), S_IRWXU | S_IRWXG );
			errorReason = errno;
	...
	{
		SdfSandboxGameState *gfxGameState = new SdfSandboxGameState( "SDF Sandbox" );

		GraphicsSystem *graphicsSystem = new SdfSandboxGraphicsSystem( gfxGameState );
	...
	const char *MainEntryPoints::getWindowTitle( void ) { return "SDF Sandbox"; }
}  // namespace Demo
